...
export default function Desk() {
  ...
  return <div className="desk"><section className="hero"><div><p className="eyebrow">ЛИЧНАЯ МЕДИА-СТУДИЯ</p><h1>Превратите слова<br /><em>в живое видео.</em></h1><p className="hero-copy">Вставьте сценарий. LumenCut озвучит его и соберёт визуальный рассказ из открытых медиа.</p></div><div className="hero-glow">✦</div></section>
    ...
    {health && <div className="health"><b>Система</b><span className={health.ffmpeg ? 'ok' : 'bad'}>● ffmpeg {health.ffmpeg ? 'готов' : 'не найден'}</span><span>LLM: {health.llmProvider}</span><span>Pexels: {health.pexels ? 'ключ есть' : 'без ключа'}</span><span>Pixabay: {health.pixabay ? 'ключ есть' : 'без ключа'}</span><span>Озвучка: {health.ttsProvider === 'lumean' ? (lumeanVoices.length ? `Lumean, голосов: ${lumeanVoices.length}` : 'Lumean — голоса не загрузились') : 'Edge TTS'}</span>{health.ttsProvider === 'lumean' && voices.length > 0 && !lumeanVoices.length && <Link to="/settings" className="settings-link">Проверьте ключ Lumean → Настройки</Link>}{(!health.pexels || !health.pixabay) && <Link to="/settings" className="settings-link">Ключи не заданы → Настройки</Link>}</div>}
