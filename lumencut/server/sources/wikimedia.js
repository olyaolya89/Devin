...
export async function search(query) {
  ...
  return Object.values(data.query?.pages || []).map(page => {
    ...
    return { source: 'wikimedia', kind: isVideo ? 'video' : 'image', url: info.url, thumbUrl: info.thumburl || info.url, width: info.width || 0, height: info.height || 0, durationSec: Number(meta.Duration?.value || 0) || null, title, description: meta.ImageDescription?.value || '', author: meta.Artist?.value || '', license: meta.LicenseShortName?.value || '', pageUrl: `https://commons.wikimedia.org/wiki/${encodeURIComponent(page.title)}` };
