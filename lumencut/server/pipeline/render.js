...
async function placeholder(scene, dir) {
  ...
  await run(['-y', '-f', 'lavfi', '-i', 'color=c=0x151c2b:s=1920x1080:d=1', '-vf', `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:textfile=${textFile}:fontcolor=white:fontsize=44:line_spacing=8:box=1:boxcolor=0x00000099:boxborderw=24:x=(w-text_w)/2:y=(h-text_h)/2`, '-frames:v', '1', file]);
  return file;
}
...
export async function render(project, dir) {
  ...
  for (const scene of project.scenes) {
    ...
    const subtitle = `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:textfile=${subtitleFile}:fontcolor=white:fontsize=44:line_spacing=8:box=1:boxcolor=0x00000099:boxborderw=12:x=(w-text_w)/2:y=h-text_h-80`;
    if (scene.clipPath) {
